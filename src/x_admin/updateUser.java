...
public class updateUser extends javax.swing.JFrame {
    ...
    public void fillFields(String id, String name, String email, String userType, String imagePath) {
        iddd.setText(id);      
        nameee.setText(name);  
        emaill.setText(email); 
        type.setSelectedItem(userType); 

        // Set the global destination variable so the SAVE button knows where the image is
        this.destination = imagePath; 

        // Display the image in the 'image' JLabel
        if (imagePath != null && !imagePath.isEmpty()) {
            config conf = new config();
            image.setIcon(conf.resizeImage(imagePath, null, image.getWidth(), image.getHeight()));
    ...
    private void initComponents() {
        ...
        jPanel3.add(totp4, new org.netbeans.lib.awtextra.AbsoluteConstraints(390, 10, 80, 50));
    ...
    String id = iddd.getText();
    if(id.isEmpty()){
        JOptionPane.showMessageDialog(null, "No User ID found!");
        return;
    }
    
    String selectedID = iddd.getText(); 
    String sql = "UPDATE tbl_user SET u_name = ?, u_email = ?, u_type = ?, u_image = ? WHERE u_id = ?";

    // 2. You MUST bind the values in the SAME ORDER as the '?' above
    conf.updateRecord(sql, 
        nameee.getText(),          // fills 1st ?
        emaill.getText(),          // fills 2nd ?
        type.getSelectedItem().toString(), // fills 3rd ?
        this.destination,               // fills 4th ? (The image path)
        id             // fills 5th ? (The ID for WHERE clause)
    );
    
    JOptionPane.showMessageDialog(null, "Updated Successfully!");
    
    // 3. Refresh the table in the main screen
